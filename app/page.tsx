...
function AuthScreenView({ authMode, setAuthMode, onAuth, onRoleSelect, isActionLoading, showToast, onGoogleAuth, onForgotPassword, onLogout, user }: any) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      // ตรวจจับ Line, Facebook, Instagram in-app browsers
      const inAppPatterns = ['Line', 'FBAV', 'FBAN', 'Instagram'];
      const isMatch = inAppPatterns.some(pattern => ua.includes(pattern));
      ...
      <div className="w-full animate-in fade-in duration-700 pb-10">
        ...
        {authMode === 'role_setup' ? (
          ...
          <>
            ...
            {isInAppBrowser && (
              <div className="w-full mb-6 animate-in fade-in">
                <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl flex gap-3 shadow-sm items-start">
                  <AlertTriangle className="text-amber-500 shrink-0 mt-0.5" size={20} />
                  <div className="text-left text-xs text-amber-800 leading-relaxed">
                    <span className="font-bold block mb-0.5">⚠️ คุณกำลังใช้งานผ่านแอปแชท</span>
                    เพื่อความปลอดภัยในการล็อกอินด้วย Google กรุณากดไอคอนจุด 3 จุดมุมหน้าจอ แล้วเลือก <strong>'เปิดในเบราว์เซอร์ (Open in Browser/Safari/Chrome)'</strong>
                  </div>
                </div>
              </div>
            )}

            {/* Google Sign-In (Hidden if in In-App Browser) */}
            {!isInAppBrowser && (
