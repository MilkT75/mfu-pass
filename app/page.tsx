  ...
  return (
    <div className={`bg-${color}-600 text-white p-10 pt-16 rounded-b-[4rem] shadow-xl relative overflow-hidden mb-8`}>
      ...
      <div className="relative z-10 flex justify-between items-start">
        ...
        {onLogout && (
          <button onClick={onLogout} className="bg-white/10 p-3 rounded-2xl hover:bg-white/20 active:scale-90 transition-all backdrop-blur-md border border-white/20">
            <LogOut size={20} />
          </button>
        )}
      </div>
...
function AuthScreenView({ authMode, setAuthMode, onAuth, onRoleSelect, isActionLoading }: any) {
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] flex flex-col items-center justify-center p-6 font-sans">
      <div className="w-full max-w-[400px] animate-in fade-in zoom-in duration-1000">
        <div className="flex flex-col items-center mb-12">
          <div className="w-24 h-24 bg-indigo-600 rounded-[2.2rem] flex items-center justify-center shadow-2xl shadow-indigo-100 mb-6 rotate-3">
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-8 font-sans pb-32 overflow-y-auto">
      <div className="flex justify-between items-center mb-12 pt-6">
        ...
        <button onClick={onLogout} className="bg-white/10 p-3 rounded-2xl border border-white/10"><LogOut size={20}/></button>
      </div>

      <div className="space-y-12">
        <section>
          <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6 flex items-center gap-2 animate-in fade-in"><Settings size={14}/> Settings</h3>
          <Card className="bg-slate-900 border-slate-800 text-white p-8 shadow-2xl">
             <p className="text-sm font-bold mb-4 text-indigo-300 flex items-center gap-2"><ImageIcon size={16}/> PromptPay QR Setup</p>
             <textarea 
             ...
             <button 
               ...
               className="w-full bg-indigo-600 py-4 rounded-2xl font-black flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"
             >
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] flex flex-col font-sans pb-20 overflow-x-hidden">
      ...
      <div className="px-6 -mt-12 flex-1 space-y-8 animate-in slide-in-from-bottom-12 duration-1000">
        <Card className="flex flex-col items-center p-10 relative overflow-hidden">
           {activePass ? (
             ...
             <div className="py-12 text-center w-full animate-in fade-in">
                ...
                <p className="text-slate-400 font-medium px-8 leading-relaxed">เรากำลังตรวจสอบยอดเงินของคุณ โปรดรอประมาณ 5-10 นาทีครับ</p>
             </div>
           ) : (
             <div className="py-10 text-center w-full animate-in zoom-in duration-500">
                ...
                <h3 className="text-3xl font-black text-slate-800 mb-3 tracking-tight">Wallet Empty</h3>
                <p className="text-slate-400 font-medium mb-10 px-6 leading-relaxed">ซื้อพาสส่วนลด 5 ใบ (มูลค่า 100.-) ในราคาเพียง 79.- เพื่อรับส่วนลดอาหารมื้อละ 20 บาท</p>
                <button onClick={onBuyPass} className="w-full bg-indigo-600 text-white font-black py-6 rounded-[2rem] shadow-2xl shadow-indigo-100 text-xl active:scale-95 transition-all">
                  BUY PASS (79.-)
                </button>
             </div>
           )}
        ...
        <div className="grid grid-cols-2 gap-5">
           <Card className="flex flex-col items-center gap-3 opacity-30 grayscale p-8 border border-slate-100"><History size={32}/><p className="text-[10px] font-black tracking-widest">HISTORY</p></Card>
           <Card className="flex flex-col items-center gap-3 opacity-30 grayscale p-8 border border-slate-100"><Info size={32}/><p className="text-[10px] font-black tracking-widest">GUIDE</p></Card>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-orange-50 flex flex-col font-sans pb-20">
      ...
      <div className="px-6 -mt-12 flex-1 space-y-8 animate-in slide-in-from-bottom-12 duration-1000">
        {!userData?.isApproved ? (
          ...
          <>
            <Card className="text-center border-none shadow-orange-200/40 p-12 relative overflow-hidden">
               <div className="absolute -top-10 -left-10 w-40 h-40 bg-orange-50 rounded-full blur-3xl"></div>
               <p className="text-slate-400 font-black text-xs uppercase mb-3 tracking-widest relative z-10">Redeemed Today</p>
               ...
               <div className="bg-orange-600 text-white py-6 rounded-[2.5rem] shadow-2xl relative z-10 animate-in zoom-in duration-700">
                  <p className="text-orange-100 text-[10px] font-black uppercase mb-1 tracking-widest text-center">Total Revenue</p>
            ...
            <Card className="bg-white border-2 border-dashed border-orange-100 text-center">
               <p className="text-slate-400 font-bold text-xs uppercase mb-4 tracking-widest">Store QR / ID</p>
...
function BuyPassView({ settings, onBack, onConfirm, isActionLoading }: any) {
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] p-8 flex flex-col font-sans max-w-lg mx-auto w-full">
      <button onClick={onBack} className="mb-10 flex items-center gap-3 text-slate-400 font-black text-xs uppercase tracking-widest active:scale-90 transition-all"><ChevronLeft size={20}/> Back</button>
      <h2 className="text-4xl font-black mb-10 italic tracking-tighter text-slate-900 animate-in fade-in duration-500">Get Digital Pass</h2>
      
      <Card className="text-center mb-8 border-indigo-100 border-2 p-10 shadow-indigo-50 animate-in slide-in-from-top-4 duration-700">
        <p className="text-indigo-600 font-black text-[11px] uppercase tracking-[0.3em] mb-6">Payment Transfer (79.00 บาท)</p>
        <div className="bg-slate-50 aspect-square rounded-[3rem] mb-8 flex items-center justify-center border-4 border-dashed border-slate-100 overflow-hidden shadow-inner p-4 relative group">
          {settings?.promptPayQr ? (
            ...
            <div className="text-center p-10 opacity-20"><QrCode size={80} className="mx-auto mb-4"/><p className="text-xs font-bold uppercase tracking-widest">Waiting for Admin QR</p></div>
          )}
        </div>
        <p className="text-slate-400 font-bold text-sm uppercase tracking-widest">MFU Pass Official Account</p>
      </Card>

      <Card className="flex-1 flex flex-col p-10 border border-slate-200 animate-in slide-in-from-bottom-4 duration-700">
        <p className="font-black text-slate-800 mb-6 text-xl tracking-tight text-center">อัปโหลดสลิปยืนยัน</p>
        <label className="flex-1 border-4 border-dashed border-slate-100 rounded-[3rem] flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 transition-all overflow-hidden relative group">
          {slip ? <img src={slip} className="w-full h-full object-cover animate-in fade-in" alt="preview"/> : (
             <div className="text-center"><Upload className="text-slate-200 mx-auto mb-4 group-hover:text-indigo-400 transition-colors" size={64}/><p className="text-slate-300 text-xs font-black uppercase tracking-widest">เลือกรูปสลิปจากอัลบั้ม</p></div>
          )}
          <input type="file" accept="image/*" className="hidden" onChange={handleFile} />
        </label>
      </Card>
      <button onClick={() => onConfirm(slip || "")} disabled={!slip || isActionLoading} className="w-full bg-indigo-600 text-white font-black py-7 rounded-[2.5rem] shadow-2xl shadow-indigo-100 mt-10 disabled:opacity-30 active:scale-95 transition-all text-xl uppercase tracking-widest">
...
function ScanQRView({ onBack, activePass, user, onSuccess }: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-10 flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="w-full aspect-square border-4 border-indigo-500 rounded-[4rem] mb-12 relative flex items-center justify-center overflow-hidden bg-black shadow-[0_0_120px_rgba(99,102,241,0.25)] animate-in zoom-in duration-700">
      ...
      <div className="w-full bg-slate-900 p-10 rounded-[3.5rem] border border-slate-800 shadow-2xl animate-in slide-in-from-bottom-8">
         <p className="text-xs font-black text-indigo-400 uppercase mb-5 text-center tracking-[0.3em]">Enter Shop ID</p>
         <input 
         ...
         <button 
           onClick={async () => {
             if (shopId && activePass?.remainingCoupons > 0) {
               const db = getFirestore();
           ...
           className="w-full bg-indigo-600 py-5 rounded-2xl mt-8 font-black uppercase text-base tracking-widest active:scale-95 transition-all shadow-xl shadow-indigo-900/40"
         >
           Confirm Redemption
         </button>
      </div>
      <button onClick={onBack} className="mt-12 text-slate-500 font-black uppercase text-xs tracking-widest hover:text-white transition-colors active:scale-90">Cancel</button>
    </div>
  );
}
...
function SuccessView({ onDone }: any) {
  ...
  return (
    <div className="min-h-screen bg-green-500 text-white p-12 flex flex-col items-center justify-center text-center animate-in fade-in duration-1000">
      <div className="bg-white text-green-500 p-10 rounded-full mb-12 shadow-2xl animate-bounce border-8 border-green-400/20">
      ...
      <div className="bg-black/15 p-10 rounded-[4rem] backdrop-blur-xl mb-16 w-full max-w-sm border border-white/20 shadow-2xl animate-in zoom-in duration-700 delay-300">
        <p className="text-xs font-black tracking-[0.3em] mb-4 opacity-70 uppercase">Discount Applied</p>
      ...
      <button onClick={onDone} className="w-full bg-white text-green-600 font-black py-7 rounded-[2.5rem] text-2xl shadow-2xl active:scale-95 hover:bg-green-50 transition-all uppercase tracking-widest animate-in slide-in-from-bottom-12">Done</button>
    </div>
  );
}